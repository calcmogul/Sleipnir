...
# TODO: Better initial centering/initial guess of decision variables w/r/t
...
def plot_solution(X: np.array, Z: np.array, U: np.array):
    ...
    ax.plot(
        times[:-1],
        [norm(U[:, k]) * math.exp(Z[0, k]) / T_max * 100 for k in range(N)],
        label="Thrust",
    )
    ax.legend()
    ...
    ax.plot(
        times,
        [
            math.degrees(math.atan2(X[0, k], math.hypot(X[1, k], X[2, k])))
            for k in range(N + 1)
        ],
        label="Glide angle",
    )
...
def main():
    ...
    for k in range(N):
        ...
        #   ρ₁ ≤ |u| exp(z) ≤ ρ₂
        #   ρ₁/exp(z) ≤ |u| ≤ ρ₂/exp(z)
        u_min = ρ_1 / math.exp(z_k.value())
        u_max = ρ_2 / math.exp(z_k.value())
