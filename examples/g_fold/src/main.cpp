...
int main() {
  ...
  // Initial position
  problem.subject_to(q(_, {_, 1}) == q_0);

  // Initial velocity
  problem.subject_to(v(_, {_, 1}) == v_0);

  // Initial ln(mass)
  problem.subject_to(Z(0, 0) == std::log(m_0));

  // Final x position
  problem.subject_to(q(0, N) == q_f(0, 0));
  ...
  for (int k = 0; k < N + 1; ++k) {
    for (int i = 0; i < 3; ++i) {
      q(i, k).set_value(std::lerp(q_0(i, 0), q_f(i, 0), k / N));
      v(i, k).set_value(std::lerp(v_0(i, 0), v_f(i, 0), k / N));
    }
  }
  ...
  for (int k = 0; k < N + 1; ++k) {
    ...
    [[maybe_unused]]
    auto x_k = X(_, {k, k + 1});
    auto q_k = X({_, 3}, {k, k + 1});
    auto v_k = X({3, 6}, {k, k + 1});
    auto z_k = Z(_, {k, k + 1});
    ...
    //
    //   std::hypot((r − r_f)₂, (r − r_f)₃) − (r − r_f)₁/std::tan(γ_gs) ≤ 0
    //   std::hypot((r − r_f)₂, (r − r_f)₃) ≤ (r − r_f)₁/std::tan(γ_gs)
    //   (r − r_f)₁/std::tan(γ_gs) ≥ std::hypot((r − r_f)₂, (r − r_f)₃)
    //   (r − r_f)₁²/tan²(γ_gs) ≥ (r − r_f)₂² + (r − r_f)₃²
    //   (r − r_f)₁² ≥ tan²(γ_gs)((r − r_f)₂² + (r − r_f)₃²)
    problem.subject_to(slp::pow(q_k(0, 0) - q_f(0, 0), 2) >=
                       std::tan(γ_gs) * std::tan(γ_gs) *
                           (slp::pow(q_k(1, 0) - q_f(1, 0), 2) +
                            slp::pow(q_k(2, 0) - q_f(2, 0), 2)));
  ...
  for (int k = 0; k < N; ++k) {
    ...
    //   ρ₁ ≤ |T_c| ≤ ρ₂
    //   ρ₁ ≤ |u| std::exp(z) ≤ ρ₂
    //   ρ₁/std::exp(z) ≤ |u| ≤ ρ₂/std::exp(z)
    double u_min = ρ_1 / std::exp(z_k(0, 0).value());
    double u_max = ρ_2 / std::exp(z_k(0, 0).value());
    ...
    // See equation (34) of [1].
    //
    //   n̂ᵀu ≥ std::cos(θ)σ where n̂ = [1  0  0]ᵀ
    //   [1  0  0]u ≥ std::cos(θ)σ
    //   u_x ≥ std::cos(θ)σ
    problem.subject_to(u_k(0, 0) >= std::cos(θ) * σ_k);
    ...
    auto σ_min =
        μ_1 * (1 - (z_k(0, 0) - z_0) + 0.5 * slp::pow(z_k(0, 0) - z_0, 2));
    auto σ_max = μ_2 * (1 - (z_k(0, 0) - z_0));
  ...
  for (int k = 0; k < N; ++k) {
    auto x_k = X(_, {k, k + 1});
    auto z_k = Z(_, {k, k + 1});
    auto x_k1 = X(_, {k + 1, k + 2});
    auto z_k1 = Z(_, {k + 1, k + 2});
  ...
  problem.minimize(slp::pow(q(1, N) - q_f(1, 0), 2) +
                   slp::pow(q(2, N) - q_f(2, 0), 2));
  problem.solve({.diagnostics = true});

  // Problem 4 from [1]: Minimum fuel
  if (0) {
    auto error_sq = slp::pow(X(1, N).value() - q_f(1, 0), 2) +
                    slp::pow(X(2, N).value() - q_f(2, 0), 2);
    problem.subject_to(slp::pow(q(1, N) - q_f(1, 0), 2) +
                           slp::pow(q(2, N) - q_f(2, 0), 2) <=
                       error_sq);
