...
void SimplicialCholeskyBase<Derived>::factorize_preordered(const CholMatrixType& ap) {
  ...
  m_maxRegularization = DiagonalScalar(0);

  Scalar apInfNorm = ap.diagonal().template lpNorm<Infinity>();
  Scalar zeroTol = std::numeric_limits<Scalar>::epsilon() * std::max(RealScalar(1), apInfNorm);
    ...
    if (DoLDLT) {
      if (signs[k] == 1) {
        ...
        if (d < RealScalar(0)) {
          ...
          m_diag[k] = apInfNorm > RealScalar(0) ? RealScalar(1) : std::numeric_limits<Scalar>::epsilon();
        } else {
      ...
      m_maxRegularization = std::max(m_maxRegularization, numext::abs(m_diag[k] - d));
    } else {
