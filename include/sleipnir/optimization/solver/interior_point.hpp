...
#include "sleipnir/optimization/regularized_ldlt.hpp"
...
interior_point(const InteriorPointMatrixCallbacks<Scalar>& matrix_callbacks,
  ...
  const Scalar μ_min = Scalar(options.tolerance) / Scalar(10.0);
  ...
  auto update_barrier_parameter_and_reset_filter = [&] {
    ...
    // See equation (7) of [2].
    using std::pow;
    μ = std::max(μ_min, std::min(κ_μ * μ, pow(μ, θ_μ)));  // NOLINT
  ...
  while (E_0 > Scalar(options.tolerance)) {
    ...
    while (1) {
      ...
      if (options.feasible_ipm && c_i.cwiseGreater(Scalar(0.0)).all()) {
          ...
          scope_exit soc_exit{[&] {
            ...
            if (options.diagnostics) {
              print_iteration_diagnostics(
                  ...
                  α_soc, Scalar(1.0), α_reduction_factor, α_z_soc);
            }
          }};
      ...
      if (full_step_rejected_counter >= 4 &&
          filter.max_constraint_violation >
              filter.back().constraint_violation / Scalar(10.0)) {
    ...
    for (int row = 0; row < z.rows(); ++row) {
      ...
      z[row] =
          std::clamp(z[row], Scalar(1.0) / κ_Σ * μ / s[row], κ_Σ * μ / s[row]);
    }
