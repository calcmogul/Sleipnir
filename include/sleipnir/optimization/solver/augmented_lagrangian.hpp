...
ExitStatus augmented_lagrangian(
  ...
  auto& c_e_prof = solve_profilers[12];
  auto& A_e_prof = solve_profilers[13];
  auto& c_i_prof = solve_profilers[14];
  auto& A_i_prof = solve_profilers[15];
  ...
  KKTSolver<Scalar> solver{
      ...
      H.nonZeros() < 0.25 * H.rows() * H.rows(),
      matrices.num_decision_variables, 0};
  ...
  if (options.diagnostics) {
    print_augmented_lagrangian_initial_iterate_diagnostics(
        E_0, f, c_e.template lpNorm<1>() + c_i.template lpNorm<1>(),
        Scalar(z.transpose() * c_i), ρ);
  }
  ...
  while (E_0 > Scalar(options.tolerance)) {
    ...
    if (options.diagnostics) {
      print_augmented_lagrangian_iteration_diagnostics(
          ...
          Scalar(z.transpose() * c_i), ρ, solver.hessian_regularization(),
          step.p_x.template lpNorm<Eigen::Infinity>(), Scalar(0), α, α_max,
