...
class SLEIPNIR_DLLEXPORT OCP : public Problem<Scalar> {
 public:
  /**
  ...
  OCP(int num_states, int num_inputs, std::chrono::duration<Scalar> dt,
      ...
      : OCP{num_states,
            ...
            [=]([[maybe_unused]] const VariableMatrix<Scalar>& t,
                ...
                const VariableMatrix<Scalar>& u,
                [[maybe_unused]]
                const VariableMatrix<Scalar>& dt) -> VariableMatrix<Scalar> {
              return dynamics(x, u);
            },
            dynamics_type,
            timestep_method,
 ...
 private:
  ...
  State rk4(F&& f, State x, Input u, Time t0, Time dt) {
    ...
    return x +
           (k1 + k2 * Scalar(2.0) + k3 * Scalar(2.0) + k4) * (dt / Scalar(6.0));
  }
