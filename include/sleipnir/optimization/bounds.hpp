...
Bounds<Scalar> get_bounds(
  ...
  for (typename decltype(inequality_constraints)::size_type constraint_index =
    ...
    if (decision_variable_value != Scalar(0.0)) {
      decision_variables[decision_variable_index].set_value(Scalar(0.0));
    ...
    if (constraint_coefficient < Scalar(0.0) && detected_bound < upper_bound) {
      upper_bound = detected_bound;
      upper_index = constraint_index;
    } else if (constraint_coefficient > Scalar(0.0) &&
...
void project_onto_bounds(
  ...
  slp_assert(κ_1 > Scalar(0.0) && κ_2 > Scalar(0.0) && κ_2 < Scalar(0.5));
  ...
  for (const auto& [lower, upper] : decision_variable_indices_to_bounds) {
    ...
    if (isfinite(lower) && isfinite(upper)) {                       // NOLINT
      auto p_L = std::min(κ_1 * std::max(Scalar(1.0), abs(lower)),  // NOLINT
                          κ_2 * (upper - lower));
      auto p_U = std::min(κ_1 * std::max(Scalar(1.0), abs(upper)),  // NOLINT
                          κ_2 * (upper - lower));
      x_i = std::min(std::max(lower + p_L, x_i), upper - p_U);
    } else if (isfinite(lower)) {  // NOLINT
      x_i = std::max(
          x_i, lower + κ_1 * std::max(Scalar(1.0), abs(lower)));  // NOLINT
    } else if (isfinite(upper)) {                                 // NOLINT
      x_i = std::min(
          x_i, upper - κ_1 * std::max(Scalar(1.0), abs(upper)));  // NOLINT
    }
  }
}
}  // namespace slp
