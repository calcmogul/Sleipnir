...
std::string power_of_10(Scalar value) {
  if (value == Scalar(0.0)) {
    ...
    using std::log10;
    int exponent = static_cast<int>(log10(value));  // NOLINT
...
void print_iteration_diagnostics(int iterations, IterationType type,
  ...
  //   x = ln(α/αᵐᵃˣ)/ln(r)
  using std::log;
  int backtracks = static_cast<int>(log(primal_α / primal_α_max) /  // NOLINT
                                    log(α_reduction_factor));       // NOLINT
