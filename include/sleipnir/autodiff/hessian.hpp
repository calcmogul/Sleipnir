...
class Hessian {
 public:
  ...
  Hessian(Variable<Scalar> variable, SleipnirMatrixLike<Scalar> auto wrt)
      : m_variable{std::move(variable)}, m_wrt{std::move(wrt)}, m_graph{[&] {
          ...
          return detail::AdjointExpressionGraph<Scalar>{m_variable};
        }()} {
    // Reset col to -1
    for (auto& node : m_wrt) {
      node.expr->col = -1;
    ...
    if (m_variable.type() <= ExpressionType::QUADRATIC) {
      ...
      m_graph.template append_hessian_triplets<UpLo>(triplets, m_wrt);
  ...
  VariableMatrix<Scalar> get() const {
    ...
    auto H = m_graph.template generate_hessian_tree<UpLo>(m_wrt);
 ...
 private:
  ...
  detail::AdjointExpressionGraph<Scalar> m_graph;
