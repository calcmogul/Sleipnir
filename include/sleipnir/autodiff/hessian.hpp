 ...
 private:
  ...
  VariableMatrix<Scalar> hessian_tree() const {
    ...
    for (size_t i = 0; i < m_top_list_end; ++i) {
      ...
      auto old = v_i->adjoint_expr;
      v_i->adjoint_expr = ptr_1;
      detail::ExpressionPtr<Scalar> g_l = v_i->grad_expr_l(v_l, v_r);
      detail::ExpressionPtr<Scalar> g_r;
      if (v_r != nullptr) {
        g_r = v_i->grad_expr_r(v_l, v_r);
      }
      v_i->adjoint_expr = old;
      ...
      if (v_i->adjoint_expr && v_i->type() > LINEAR) {
        if (v_r != nullptr) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT) {
            if (auto h_ll = v_i->hess_expr_ll(v_l, v_r)) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT && v_r->type() > CONSTANT) {
            if (auto h_lr = v_i->hess_expr_lr(v_l, v_r)) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_r->type() > CONSTANT) {
            if (auto h_rr = v_i->hess_expr_rr(v_l, v_r)) {
  ...
  void append_triplets(
    ...
    for (size_t i = 0; i < m_top_list_end; ++i) {
      ...
      // Compute node gradients
      auto old = v_i->adjoint;
      v_i->adjoint = S(1);
      S g_l;
      S g_r;
      if (v_r != nullptr) {
        g_l = v_i->grad_l(v_l->val, v_r->val);
        g_r = v_i->grad_r(v_l->val, v_r->val);
      } else {
        g_l = v_i->grad_l(v_l->val, S(0));
        g_r = S(0);
      }
      v_i->adjoint = old;
      ...
      if (v_i->type() > LINEAR) {
        if (v_r != nullptr) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT) {
            if (auto h_ll = v_i->hess_ll(v_l->val, v_r->val)) {
              push_edge(k_l, k_l, h_ll.value());
            } else {
#if 0
              slp::println("h_ll = 0");
              slp::println("  a(v_i) = {}: {}, {}", v_i->adjoint, v_i->name(),
                           v_i->type());
              slp::println("  v_l = {}: {}", v_l->val, v_l->type());
              slp::println("  v_r = {}: {}", v_r->val, v_r->type());
#endif
            }
          }
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT && v_r->type() > CONSTANT) {
            if (auto h_lr = v_i->hess_lr(v_l->val, v_r->val)) {
              push_edge(k_l, k_r, S(k_l == k_r ? 2 : 1) * h_lr.value());
            } else {
#if 0
              slp::println("h_lr = 0");
              slp::println("  a(v_i) = {}: {}, {}", v_i->adjoint, v_i->name(),
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_r->type() > CONSTANT) {
            if (auto h_rr = v_i->hess_rr(v_l->val, v_r->val)) {
              push_edge(k_r, k_r, h_rr.value());
            } else {
#if 0
              slp::println("h_rr = 0");
              slp::println("  (v_i) = {}: {}, {}", v_i->adjoint, v_i->name(),
                           v_i->type());
              slp::println("  v_l = {}: {}", v_l->val, v_l->type());
              slp::println("  v_r = {}: {}", v_r->val, v_r->type());
#endif
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT) {
            if (auto h_ll = v_i->hess_ll(v_l->val, S(0))) {
              push_edge(k_l, k_l, h_ll.value());
            } else {
#if 0
              slp::println("h_ll = 0");
