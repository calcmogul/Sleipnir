    ...
    for (size_t i = 0; i < m_top_list_end; ++i) {
      ...
      if (v_i->adjoint_expr && v_i->type() > LINEAR) {
        if (v_r != nullptr) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT) {
            if (auto h_ll = v_i->hess_expr_ll(v_l, v_r, ptr_1)) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT && v_r->type() > CONSTANT) {
            if (auto h_lr = v_i->hess_expr_lr(v_l, v_r, ptr_1)) {
              push_edge(
                  k_l, k_r,
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_r->type() > CONSTANT) {
            if (auto h_rr = v_i->hess_expr_rr(v_l, v_r, ptr_1)) {
    ...
    for (size_t i = 0; i < m_top_list_end; ++i) {
      ...
      if (v_i->adjoint != S(0) && v_i->type() > LINEAR) {
        if (v_r != nullptr) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT) {
            if (S h_ll = v_i->hess_ll(v_l->val, v_r->val, S(1)); h_ll != S(0)) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT && v_r->type() > CONSTANT) {
            if (S h_lr = v_i->hess_lr(v_l->val, v_r->val, S(1)); h_lr != S(0)) {
              push_edge(k_l, k_r, S(k_l == k_r ? 2 : 1) * h_lr * v_i->adjoint);
            }
          }

          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_r->type() > CONSTANT) {
            if (S h_rr = v_i->hess_rr(v_l->val, v_r->val, S(1)); h_rr != S(0)) {
          ...
          // h(vⱼ, vₖ) += ∂²ϕᵢ/∂vⱼ∂vₖ a(vᵢ)
          if (v_l->type() > CONSTANT) {
            if (S h_ll = v_i->hess_ll(v_l->val, S(0), S(1)); h_ll != S(0)) {
              push_edge(k_l, k_l, h_ll * v_i->adjoint);
            }
          }
        }
