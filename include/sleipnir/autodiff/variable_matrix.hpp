...
class SLEIPNIR_DLLEXPORT VariableMatrix : public SleipnirBase {
 public:
   ...
   * @param list The nested list of Variables.
   */
  VariableMatrix(const std::vector<std::vector<Scalar>>& list) {  // NOLINT
    // Get row and column counts for destination matrix
    m_rows = list.size();
    m_cols = 0;
    if (list.size() > 0) {
   ...
   */
  VariableMatrix(  // NOLINT
      const std::vector<std::vector<Variable<Scalar>>>& list) {
    // Get row and column counts for destination matrix
    m_rows = list.size();
    m_cols = 0;
    if (list.size() > 0) {
    ...
    for (int row = 0; row < values.rows(); ++row) {
      for (int col = 0; col < values.cols(); ++col) {
        if (row == col) {
          ...
          m_storage.emplace_back(Scalar(0.0));
        }
      }
    }
  }
   ...
   */
  VariableMatrix(const Variable<Scalar>& variable)  // NOLINT
      : m_rows{1}, m_cols{1} {
   ...
   * @param variable Variable.
   */
  VariableMatrix(Variable<Scalar>&& variable)  // NOLINT
      : m_rows{1}, m_cols{1} {
  ...
  friend SLEIPNIR_DLLEXPORT VariableMatrix<Scalar> operator*(const LHS& lhs,
                                                             const RHS& rhs) {
  ...
  friend SLEIPNIR_DLLEXPORT VariableMatrix<Scalar> operator*(
      const LHS& lhs, const Variable<Scalar>& rhs) {
  ...
  friend SLEIPNIR_DLLEXPORT VariableMatrix<Scalar> operator*(
      const Variable<Scalar>& lhs, const RHS& rhs) {
   ...
   */
  template <SleipnirMatrixLike<Scalar> LHS, SleipnirMatrixLike<Scalar> RHS>
  friend SLEIPNIR_DLLEXPORT VariableMatrix<typename LHS::Scalar> operator-(
  ...
  operator-(const SleipnirMatrixLike<Scalar> auto& lhs) {
    VariableMatrix<Scalar> result{detail::empty, lhs.rows(), lhs.cols()};
   ...
   */
  operator Variable<Scalar>() const {  // NOLINT
    slp_assert(rows() == 1 && cols() == 1);
    return (*this)[0, 0];
  }
...
VariableMatrix(std::initializer_list<std::initializer_list<Variable<Scalar>>>)
    -> VariableMatrix<Scalar>;

template <typename Scalar>
VariableMatrix(const std::vector<std::vector<Scalar>>&)
    -> VariableMatrix<Scalar>;

template <typename Scalar>
VariableMatrix(const std::vector<std::vector<Variable<Scalar>>>&)
    -> VariableMatrix<Scalar>;
...
template <typename Scalar>
VariableMatrix(const Variable<Scalar>&) -> VariableMatrix<Scalar>;

template <typename Scalar>
VariableMatrix(Variable<Scalar>&&) -> VariableMatrix<Scalar>;

template <typename Scalar>
VariableMatrix(std::span<const Variable<Scalar>>) -> VariableMatrix<Scalar>;

template <typename Scalar>
VariableMatrix(std::span<const Variable<Scalar>>, int, int)
    -> VariableMatrix<Scalar>;
 ...
 */
template <typename Scalar>
SLEIPNIR_DLLEXPORT VariableMatrix<Scalar> cwise_reduce(
 ...
 */
template <typename Scalar>
SLEIPNIR_DLLEXPORT VariableMatrix<Scalar> block(
 ...
 */
template <typename Scalar>
SLEIPNIR_DLLEXPORT VariableMatrix<Scalar> solve(
    const VariableMatrix<Scalar>& A, const VariableMatrix<Scalar>& B) {
  // m x n * n x p = m x p
