...
struct Expression {
  ...
  friend ExpressionPtr<Scalar> operator*(const ExpressionPtr<Scalar>& lhs,
    ...
    // Prune expression
    if (lhs->is_constant(Scalar(0.0))) {
      // Return zero
      return lhs;
    } else if (rhs->is_constant(Scalar(0.0))) {
      // Return zero
      return rhs;
    } else if (lhs->is_constant(Scalar(1.0))) {
      return rhs;
    } else if (rhs->is_constant(Scalar(1.0))) {
      return lhs;
    }
  ...
  friend ExpressionPtr<Scalar> operator+(const ExpressionPtr<Scalar>& lhs,
    ...
    // Prune expression
    if (lhs == nullptr || lhs->is_constant(Scalar(0.0))) {
      return rhs;
    } else if (rhs == nullptr || rhs->is_constant(Scalar(0.0))) {
      return lhs;
    }
  ...
  virtual ExpressionPtr<Scalar> grad_expr_l(
    ...
    return constant_ptr(Scalar(0.0));
  }
...
template <typename Scalar>
ExpressionPtr<Scalar> cbrt(const ExpressionPtr<Scalar>& x);  // NOLINT
template <typename Scalar>
ExpressionPtr<Scalar> exp(const ExpressionPtr<Scalar>& x);  // NOLINT
template <typename Scalar>
ExpressionPtr<Scalar> sin(const ExpressionPtr<Scalar>& x);  // NOLINT
template <typename Scalar>
ExpressionPtr<Scalar> sinh(const ExpressionPtr<Scalar>& x);
template <typename Scalar>
ExpressionPtr<Scalar> sqrt(const ExpressionPtr<Scalar>& x);  // NOLINT
...
struct CbrtExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    Scalar c = cbrt(x);  // NOLINT
    return parent_adjoint / (Scalar(3.0) * c * c);
  }
  ...
  ExpressionPtr<Scalar> grad_expr_l(
    ...
    return parent_adjoint / (constant_ptr(Scalar(3.0)) * c * c);
  }
};
...
ExpressionPtr<Scalar> cbrt(const ExpressionPtr<Scalar>& x) {  // NOLINT
  ...
  if (x->type() == CONSTANT) {
    if (x->val == Scalar(0.0)) {
      // Return zero
      return x;
    } else if (x->val == Scalar(-1.0) || x->val == Scalar(1.0)) {
      return x;
    } else {
      return constant_ptr(cbrt(x->val));  // NOLINT
    }
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> abs(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::abs;

  // Prune expression
  if (x->is_constant(Scalar(0.0))) {
    // Return zero
    return x;
  }
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(abs(x->val));  // NOLINT
  }
...
struct AcosExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    return -parent_adjoint / sqrt(Scalar(1.0) - x * x);  // NOLINT
  }
  ...
  ExpressionPtr<Scalar> grad_expr_l(
      ...
      const ExpressionPtr<Scalar>& parent_adjoint) const override {
    return -parent_adjoint /
           slp::detail::sqrt(constant_ptr(Scalar(1.0)) - x * x);
  }
};
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> acos(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::acos;

  // Prune expression
  if (x->is_constant(Scalar(0.0))) {
    return constant_ptr(Scalar(std::numbers::pi) / Scalar(2.0));
  }

  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(acos(x->val));  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> asin(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::asin;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(asin(x->val));  // NOLINT
  }
...
struct AtanExpression final : Expression<Scalar> {
  ...
  Scalar value(Scalar x, Scalar) const override {
    return atan(x);  // NOLINT
  }

  ExpressionType type() const override { return ExpressionType::NONLINEAR; }

  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    return parent_adjoint / (Scalar(1.0) + x * x);
  }

  ExpressionPtr<Scalar> grad_expr_l(
      const ExpressionPtr<Scalar>& x, const ExpressionPtr<Scalar>&,
      const ExpressionPtr<Scalar>& parent_adjoint) const override {
    return parent_adjoint / (constant_ptr(Scalar(1.0)) + x * x);
  }
};
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> atan(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::atan;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(atan(x->val));  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> atan2(const ExpressionPtr<Scalar>& y,  // NOLINT
                            const ExpressionPtr<Scalar>& x) {
  using enum ExpressionType;
  using std::atan2;

  // Prune expression
  if (y->is_constant(Scalar(0.0))) {
    // Return zero
    return y;
  } else if (x->is_constant(Scalar(0.0))) {
    return constant_ptr(Scalar(std::numbers::pi) / Scalar(2.0));
  }

  // Evaluate constant
  if (y->type() == CONSTANT && x->type() == CONSTANT) {
    return constant_ptr(atan2(y->val, x->val));  // NOLINT
  }
...
struct CosExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    return parent_adjoint * -sin(x);  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> cos(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::cos;

  // Prune expression
  if (x->is_constant(Scalar(0.0))) {
    return constant_ptr(Scalar(1.0));
  }

  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(cos(x->val));  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> cosh(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::cosh;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(cosh(x->val));  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> erf(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::erf;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(erf(x->val));  // NOLINT
  }
...
struct ExpExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    return parent_adjoint * exp(x);  // NOLINT
  }
...
ExpressionPtr<Scalar> exp(const ExpressionPtr<Scalar>& x) {  // NOLINT
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(exp(x->val));  // NOLINT
  }
...
template <typename Scalar>
ExpressionPtr<Scalar> hypot(const ExpressionPtr<Scalar>& x,  // NOLINT
...
struct HypotExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar y, Scalar parent_adjoint) const override {
    ...
    return parent_adjoint * x / hypot(x, y);  // NOLINT
  }

  Scalar grad_r(Scalar x, Scalar y, Scalar parent_adjoint) const override {
    using std::hypot;

    return parent_adjoint * y / hypot(x, y);  // NOLINT
  }
...
ExpressionPtr<Scalar> hypot(const ExpressionPtr<Scalar>& x,  // NOLINT
  ...
  if (x->is_constant(Scalar(0.0))) {
    return y;
  } else if (y->is_constant(Scalar(0.0))) {
    return x;
  }

  // Evaluate constant
  if (x->type() == CONSTANT && y->type() == CONSTANT) {
    return constant_ptr(hypot(x->val, y->val));  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> log(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::log;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(log(x->val));  // NOLINT
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> log10(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::log10;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(log10(x->val));  // NOLINT
  }
...
template <typename Scalar>
ExpressionPtr<Scalar> pow(const ExpressionPtr<Scalar>& base,  // NOLINT
...
struct PowExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar base, Scalar power,
    ...
    return parent_adjoint * pow(base, power - 1) * power;  // NOLINT
  }
  ...
  Scalar grad_r(Scalar base, Scalar power,
    ...
    if (base == Scalar(0.0)) {
      ...
      return parent_adjoint * pow(base, power) * log(base);  // NOLINT
    }
  }
  ...
  ExpressionPtr<Scalar> grad_expr_l(
      ...
      const ExpressionPtr<Scalar>& parent_adjoint) const override {
    return parent_adjoint *
           slp::detail::pow(base, power - constant_ptr(Scalar(1.0))) * power;
  }
...
ExpressionPtr<Scalar> pow(const ExpressionPtr<Scalar>& base,  // NOLINT
  ...
  // Prune expression
  if (base->is_constant(Scalar(0.0))) {
    // Return zero
    return base;
  } else if (base->is_constant(Scalar(1.0))) {
    // Return one
    return base;
  }
  if (power->is_constant(Scalar(0.0))) {
    return constant_ptr(Scalar(1.0));
  } else if (power->is_constant(Scalar(1.0))) {
    return base;
  }

  // Evaluate constant
  if (base->type() == CONSTANT && power->type() == CONSTANT) {
    return constant_ptr(pow(base->val, power->val));  // NOLINT
  }

  if (power->is_constant(Scalar(2.0))) {
    if (base->type() == LINEAR) {
...
struct SinExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    return parent_adjoint * cos(x);  // NOLINT
  }
...
ExpressionPtr<Scalar> sin(const ExpressionPtr<Scalar>& x) {  // NOLINT
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(sin(x->val));  // NOLINT
  }
...
struct SinhExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    return parent_adjoint * cosh(x);  // NOLINT
  }
...
ExpressionPtr<Scalar> sinh(const ExpressionPtr<Scalar>& x) {
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(sinh(x->val));  // NOLINT
  }
...
struct SqrtExpression final : Expression<Scalar> {
  ...
  Scalar grad_l(Scalar x, Scalar, Scalar parent_adjoint) const override {
    ...
    return parent_adjoint / (Scalar(2.0) * sqrt(x));  // NOLINT
  }

  ExpressionPtr<Scalar> grad_expr_l(
      const ExpressionPtr<Scalar>& x, const ExpressionPtr<Scalar>&,
      const ExpressionPtr<Scalar>& parent_adjoint) const override {
    return parent_adjoint / (constant_ptr(Scalar(2.0)) * slp::detail::sqrt(x));
  }
};
...
ExpressionPtr<Scalar> sqrt(const ExpressionPtr<Scalar>& x) {  // NOLINT
  ...
  if (x->type() == CONSTANT) {
    if (x->val == Scalar(0.0)) {
      // Return zero
      return x;
    } else if (x->val == Scalar(1.0)) {
      return x;
    } else {
      return constant_ptr(sqrt(x->val));  // NOLINT
    }
  }
 ...
 */
template <typename Scalar>
ExpressionPtr<Scalar> tan(const ExpressionPtr<Scalar>& x) {  // NOLINT
  using enum ExpressionType;
  using std::tan;
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(tan(x->val));  // NOLINT
  }
...
ExpressionPtr<Scalar> tanh(const ExpressionPtr<Scalar>& x) {
  ...
  // Evaluate constant
  if (x->type() == CONSTANT) {
    return constant_ptr(tanh(x->val));  // NOLINT
  }
