...
TEMPLATE_TEST_CASE("VariableMatrix - Construct from Eigen::MatrixBase",
  ...
  slp::VariableMatrix mat{expected};
...
TEMPLATE_TEST_CASE("VariableMatrix - Assignment to default", "[VariableMatrix]",
  ...
  CHECK(mat.rows() == 2);
  CHECK(mat.cols() == 2);
  CHECK(mat[0, 0].value() == T(0.0));
  CHECK(mat[0, 1].value() == T(0.0));
  CHECK(mat[1, 0].value() == T(0.0));
  CHECK(mat[1, 1].value() == T(0.0));
  ...
  CHECK(mat[0, 0].value() == T(1.0));
  CHECK(mat[0, 1].value() == T(2.0));
  CHECK(mat[1, 0].value() == T(3.0));
  CHECK(mat[1, 1].value() == T(4.0));
}
...
TEMPLATE_TEST_CASE("VariableMatrix - Assignment aliasing", "[VariableMatrix]",
  ...
  slp::VariableMatrix<T> A{{1.0, 2.0}, {3.0, 4.0}};
  slp::VariableMatrix<T> B{{5.0, 6.0}, {7.0, 8.0}};
  ...
  B[0, 0].set_value(T(2.0));
  expected_B(0, 0) = T(2.0);
  CHECK(A.value() == expected_B);
  CHECK(B.value() == expected_B);
}
...
TEMPLATE_TEST_CASE("VariableMatrix - Block() member function",
  ...
  A.block(1, 1, 2, 2).block(1, 1, 1, 1) = T(14.0);
...
TEMPLATE_TEST_CASE("VariableMatrix - Value", "[VariableMatrix]",
  ...
  CHECK(A.value(3) == T(4.0));
  CHECK(A.T().value(3) == T(2.0));

  // Block
  CHECK(A.block(1, 1, 2, 2).value() == expected.block(1, 1, 2, 2));
  CHECK(A.block(1, 1, 2, 2).value(2) == T(8.0));
  CHECK(A.T().block(1, 1, 2, 2).value(2) == T(6.0));

  // Slice
  CHECK(A[slp::Slice{1, 3}, slp::Slice{1, 3}].value() ==
        expected.block(1, 1, 2, 2));
  CHECK(A[slp::Slice{1, 3}, slp::Slice{1, 3}].value(2) == T(8.0));
  CHECK(A[slp::Slice{1, 3}, slp::Slice{1, 3}].T().value(2) == T(6.0));

  // Block-of-block
  CHECK(A.block(1, 1, 2, 2).block(0, 1, 2, 1).value() ==
        expected.block(1, 1, 2, 2).block(0, 1, 2, 1));
  CHECK(A.block(1, 1, 2, 2).block(0, 1, 2, 1).value(1) == T(9.0));
  CHECK(A.block(1, 1, 2, 2).T().block(0, 1, 2, 1).value(1) == T(9.0));
...
void check_solve(slp::VariableMatrix<T> A, slp::VariableMatrix<T> B) {
  ...
  CHECK(X.rows() == A.cols());
  CHECK(X.cols() == B.cols());
  CHECK((A.value() * X.value() - B.value()).norm() < T(1e-12));
}
