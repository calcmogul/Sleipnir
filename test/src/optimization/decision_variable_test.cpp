...
TEMPLATE_TEST_CASE("decision_variable - Vector init assign",
  ...
  CHECK(y.value(0) == T(0.0));
  CHECK(y.value(1) == T(0.0));

  // Vector assignment
  y[0].set_value(T(1.0));
  y[1].set_value(T(2.0));
  CHECK(y.value(0) == T(1.0));
  CHECK(y.value(1) == T(2.0));
  y[0].set_value(T(3.0));
  y[1].set_value(T(4.0));
  CHECK(y.value(0) == T(3.0));
  CHECK(y.value(1) == T(4.0));
}
...
TEMPLATE_TEST_CASE("decision_variable - Static matrix init assign",
  ...
  CHECK(z.value(0, 0) == T(0.0));
  CHECK(z.value(0, 1) == T(0.0));
  CHECK(z.value(1, 0) == T(0.0));
  CHECK(z.value(1, 1) == T(0.0));
  CHECK(z.value(2, 0) == T(0.0));
  CHECK(z.value(2, 1) == T(0.0));

  // Matrix assignment; element comparison
  z.set_value(Eigen::Matrix<T, 3, 2>{{T(1), T(2)}, {T(3), T(4)}, {T(5), T(6)}});
  CHECK(z.value(0, 0) == T(1.0));
  CHECK(z.value(0, 1) == T(2.0));
  CHECK(z.value(1, 0) == T(3.0));
  CHECK(z.value(1, 1) == T(4.0));
  CHECK(z.value(2, 0) == T(5.0));
  CHECK(z.value(2, 1) == T(6.0));
...
TEMPLATE_TEST_CASE("decision_variable - Symmetric matrix",
  ...
  CHECK(A.value(0, 0) == T(0.0));
  CHECK(A.value(0, 1) == T(0.0));
  CHECK(A.value(1, 0) == T(0.0));
  CHECK(A.value(1, 1) == T(0.0));

  // Assign to lower triangle
  A[0, 0].set_value(T(1.0));
  A[1, 0].set_value(T(2.0));
  A[1, 1].set_value(T(3.0));

  // Confirm whole matrix changed
  CHECK(A.value(0, 0) == T(1.0));
  CHECK(A.value(0, 1) == T(2.0));
  CHECK(A.value(1, 0) == T(2.0));
  CHECK(A.value(1, 1) == T(3.0));
}
