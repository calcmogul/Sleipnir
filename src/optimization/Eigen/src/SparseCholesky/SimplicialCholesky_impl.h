...
void SimplicialCholeskyBase<Derived>::factorize_preordered(const CholMatrixType& ap) {
  ...
  // find correct signs of permuted diagonal
  VectorType signs{ap.rows()};
  signs.segment(0, m_positiveEigenvalues).setConstant(1.0);
  signs.segment(m_positiveEigenvalues, ap.rows() - m_positiveEigenvalues).setConstant(-1.0);
  // PAP⁻¹ = LDLᵀ, so pivot signs via P
  signs = m_P * signs;
  m_maxRegularization = 0.0;

  Scalar apInfnorm = 0.0;
  for (Index col = 0; col < ap.cols(); ++col) {
    for (typename CholMatrixType::InnerIterator it{ap, col}; it; ++it) {
      apInfnorm = std::max(apInfnorm, std::abs(it.value()));
    }
  }
  Scalar zeroTol = 10.0 * ap.rows() * std::numeric_limits<Scalar>::epsilon() * apInfnorm;
    ...
    if (DoLDLT) {
      if (signs[k] == 1.0) {
        ...
        if (d < RealScalar(0)) {
          ...
          m_maxRegularization = std::max(m_maxRegularization, std::abs(d));
        } else if (d == RealScalar(0)) {
          // if it's zero, bump it to 1
          m_diag[k] = RealScalar(1);
          m_maxRegularization = std::max(m_maxRegularization, 1.0);
        } else {
