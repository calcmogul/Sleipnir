...
void print_ipm_iteration_diagnostics(
    ...
    const std::chrono::duration<Rep, Period>& time, double error, double cost,
    double infeasibility, double complementarity, double μ, double δ,
    double primal_α, double primal_α_max, double α_reduction_factor,
    double dual_α) {
  if (iterations % 20 == 0) {
    if (iterations == 0) {
      slp::print("┏");
...
void print_augmented_lagrangian_iteration_diagnostics(
    int iterations, IterationType type,
    const std::chrono::duration<Rep, Period>& time, double error, double cost,
    double infeasibility, double complementarity, double ρ, double δ,
    double primal_α, double primal_α_max, double dual_α) {
  if (iterations % 20 == 0) {
    if (iterations == 0) {
      slp::print("┏");
    } else {
